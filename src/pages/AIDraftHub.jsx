// src/pages/AIDraftHub.jsx
// AI 초안 생성 허브 — ISO 13485 QMS 문서 자동 초안
import React, { useState } from 'react'
import { Sparkles, Copy, Check, FileText, AlertCircle, ChevronDown } from 'lucide-react'
import AppLayout from '../components/AppLayout'

const DOC_TYPES = [
  {
    key: 'sop', label: '작업표준서 (SOP)', iso: '§4.2.4',
    fields: [
  ...
  {
    key: 'capa', label: 'CAPA 보고서', iso: '§8.5.2/8.5.3',
    fields: [
  ...
  {
    key: 'risk', label: '위험 평가서', iso: 'ISO 14971',
    fields: [
  ...
  {
    key: 'complaint', label: '고객불만 조사보고서', iso: '§8.2.1',
    fields: [
      { key: 'type', label: '불만 유형', placeholder: '예: 제품 오작동, 포장 불량, 배송 오류' },
  ...
  {
    key: 'change', label: '변경요청서 (CCR)', iso: '§4.1.4',
    fields: [
  ...
  {
    key: 'supplier', label: '공급업체 평가보고서', iso: '§7.4.1',
    fields: [
  ...
  {
    key: 'qm', label: '품질매뉴얼 섹션', iso: '§4.2.2',
    fields: [
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (data.ok) {
        ...
        setError(data.error || data.message || '생성 실패')
      }
    } catch (e) {
      setError(e.message)
    } finally {
    ...
    <AppLayout>
      <div style={{ padding: '24px', maxWidth: 900, margin: '0 auto' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: 20, alignItems: 'start' }}>
          {/* 문서 유형 선택 */}
          <div>
            <div style={{ fontSize: 12, fontWeight: 600, color: 'var(--ink-faint)', textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 8 }}>
              문서 유형 선택
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {DOC_TYPES.map(dt => (
                <button key={dt.key}
                  onClick={() => { setSelectedType(dt.key); setFieldValues({}); setResult(null); setError(null) }}
                  style={{
                    textAlign: 'left', padding: '10px 14px', borderRadius: 8, border: '1px solid',
                    borderColor: selectedType === dt.key ? 'var(--moss)' : 'var(--line)',
                    background: selectedType === dt.key ? 'rgba(var(--moss-rgb,60,130,90),0.08)' : 'var(--surface)',
                    cursor: 'pointer', transition: 'all 0.15s'
                  }}>
                  <div style={{ fontSize: 13, fontWeight: selectedType === dt.key ? 600 : 400, color: selectedType === dt.key ? 'var(--moss)' : 'var(--ink)' }}>
                    {dt.label}
                  </div>
            ...
            {!selectedType ? (
              ...
              <div style={{ background: 'var(--surface)', border: '1px solid var(--line)', borderRadius: 12, padding: 24 }}>
                <h2 style={{ fontSize: 15, fontWeight: 600, margin: '0 0 16px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <FileText size={16} style={{ color: 'var(--moss)' }} />
                  {docType.label}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 20 }}>
                  {docType.fields.map(f => (
                    <div key={f.key}>
                      <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--ink-faint)', marginBottom: 4 }}>
                        {f.label}
                      </label>
                      <input
                        type="text"
                        ...
                        style={{ width: '100%', padding: '9px 12px', border: '1px solid var(--line)', borderRadius: 8, fontSize: 13, boxSizing: 'border-box', background: 'var(--bg)' }}
                      />
                    </div>
                  ))}
                </div>
                ...
                <button
                  ...
                  style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 24px',
                    ...
                    border: 'none', borderRadius: 8, cursor: allFilled && !loading ? 'pointer' : 'not-allowed',
                ...
                {error && (
                  <div style={{ marginTop: 16, padding: '12px 16px', background: '#fef2f2', borderRadius: 8, border: '1px solid #fecaca', display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                ...
                {result && (
                  <div style={{ marginTop: 20 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
                      ...
                      <button onClick={handleCopy}
                        style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '5px 12px',
                          background: copied ? '#f0fdf4' : 'var(--bg)', border: '1px solid var(--line)',
                          borderRadius: 6, cursor: 'pointer', fontSize: 12, color: copied ? '#059669' : 'var(--ink)' }}>
                    ...
                    <p style={{ fontSize: 11, color: 'var(--ink-faint)', marginTop: 8 }}>
                      ⚠️ AI가 생성한 초안입니다. 반드시 내용을 검토·수정한 후 공식 기록으로 저장하세요.
                    </p>
                  </div>
                )}
              </div>
