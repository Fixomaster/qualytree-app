...
import { logs as logisticsLogs, adverseEvents, LOG_TYPE } from '../../lib/logisticsState'
import { complaints, reviews } from '../../lib/managementReviewState'
import { capa } from '../../lib/capaState'
import { sessions as trainingSessions } from '../../lib/trainingState'
import { audits } from '../../lib/internalAuditState'

const DOC_KEY = 'qualytree.documents'

// 필수 절차서 — 신규 추가 시 실제 procedures 목록에도 자동 반영(보완)되는 항목
const REQUIRED_NEW_PROCEDURES = ['공급업체관리 절차서', '회수(Recall) 절차서', '변경관리 절차서']
...
export default function KgmpHub() {
  ...
  // 필수 절차서 중 아직 실제 절차 목록에 없는 항목(공급업체관리·회수·변경관리)을 자동 보완
  useEffect(() => {
    onboarding.ensureProcedures(REQUIRED_NEW_PROCEDURES)
    setTick((x) => x + 1)
  ...
  const sections = [
    ...
    },
    {
      ...
      icon: BookOpen,
      items: REQUIRED_PROCEDURES.map(procedureItem),
    },
    {
      id: 'records',
      title: '유지해야 하는 기록',
